...
var currentProgramPath string
var currentProgramSource string

type scriptBuildOptions struct {
	showConsole bool
}
...
type VM struct {
	...
	sp           int
	globals      [globalsLimit]TaggedValue
...
func (vm *VM) SetExecutionLimits(maxOps int, timeout time.Duration) {
	vm.maxOps = maxOps
	if timeout > 0 {
...
func (vm *VM) SetBlockDangerousSideEffects(block bool) {
	vm.blockDanger = block
}

func (vm *VM) SetProgramContext(path string, source string) {
	vm.programPath = path
	vm.programSource = source
...
func handleOpGetGlobal(vm *VM, fr *frame) error {
	...
	return vm.pushTagged(vm.globals[slot])
}

func handleOpSetGlobal(vm *VM, fr *frame) error {
	slot := vm.readUint16(fr)
	value, err := vm.popTagged()
	if err != nil {
		return err
	}
...
func handleOpGetLocal(vm *VM, fr *frame) error {
	slot := vm.readUint16(fr)
	return vm.pushTagged(vm.readLocalTagged(fr.basePointer + slot))
}
...
func handleOpSetLocal(vm *VM, fr *frame) error {
	...
	vm.writeLocalTagged(fr.basePointer+slot, value)
	return nil
}
...
func handleOpEqual(vm *VM, fr *frame) error {
	...
	rv := rtv.ToAny()
	vm.sp--
	vm.stack[vm.sp-1] = NewBool(safeEqualValues(lv, rv))
	return nil
}
...
func safeEqualValues(left, right Value) bool {
	defer func() {
		_ = recover()
	}()
	return left == right
...
func (vm *VM) checkExecutionBudget() error {
	if !vm.deadline.IsZero() && time.Now().After(vm.deadline) {
		return vm.runtimeError("execution timeout")
	}
	if vm.maxOps > 0 {
		vm.opCount++
		if vm.opCount > vm.maxOps {
			return vm.runtimeError("execution instruction limit exceeded")
		}
	}
	return nil
}
...
func (vm *VM) runLoop() error {
	if vm.cancel == nil && !vm.profiling {
		return vm.runLoopFast()
	}
	for vm.fp > 0 {
		if err := vm.checkExecutionBudget(); err != nil {
			return err
		}
		if vm.cancel != nil {
			select {
		...
		case bytecode.OpGetGlobal:
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpGetGlobal")
			}
			slot := int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
			fr.ip += 2
			sp := vm.sp
			if sp >= stackLimit {
				return vm.runtimeError("stack overflow")
			}
			vm.stack[sp] = vm.globals[slot]
			vm.sp = sp + 1
		case bytecode.OpSetGlobal:
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpSetGlobal")
			}
			slot := int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
			fr.ip += 2
			sp := vm.sp
			...
			sp--
			vm.globals[slot] = vm.stack[sp]
			vm.stack[sp] = TaggedValue{}
			vm.sp = sp
		case bytecode.OpGetLocal:
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpGetLocal")
			}
			slot := int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
			fr.ip += 2
			sp := vm.sp
			...
			vm.stack[sp] = vm.readLocalTagged(fr.basePointer + slot)
			vm.sp = sp + 1
		case bytecode.OpSetLocal:
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpSetLocal")
			}
			slot := int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
			fr.ip += 2
			sp := vm.sp
		...
		case bytecode.OpJump:
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpJump")
			}
			fr.ip = int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
		case bytecode.OpJumpIfFalse:
			if vm.sp < 1 {
				return vm.runtimeError("stack underflow")
			}
			if fr.ip+1 >= len(fr.fn().Code) {
				return vm.runtimeError("truncated operand for OpJumpIfFalse")
			}
			target := int(fr.fn().Code[fr.ip])<<8 | int(fr.fn().Code[fr.ip+1])
			fr.ip += 2
			tv := vm.stack[vm.sp-1]
			vm.sp--
			shouldJump := false
...
func (vm *VM) call(argc int) error {
	...
	case builtinValue:
		if vm.blockDanger && (fn.name == "os.exit" || fn.name == "subprocess.run") {
			return vm.runtimeError("%s is blocked in safe execution mode", fn.name)
		}
...
func runCallableOnFreshVM(ctx *VM, callable Value, args []Value, cancel <-chan struct{}) (Value, error) {
	...
	if ctx != nil {
		...
		worker.blockDanger = ctx.blockDanger
		worker.maxOps = ctx.maxOps
		if !ctx.deadline.IsZero() {
			worker.deadline = ctx.deadline
		}
	}
...
func spawnTask(ctx *VM, callable Value, args []Value) *taskValue {
	taskRuntimeOnce.Do(initTaskRuntime)
	task := newTaskValue()
	...
	case taskQueue <- job:
	default:
		// If the queue is saturated, run this task in a fallback goroutine
		// to avoid deadlocks under heavy fan-out.
		go taskWorkerExecute(job)
	}
	return task
}

func taskWorkerExecute(job taskJob) {
	select {
	case <-job.task.cancelCh:
		job.task.complete(nil, errTaskCanceled)
		return
	default:
	}
	defer func() {
		if recovered := recover(); recovered != nil {
			job.task.complete(nil, fmt.Errorf("task panic: %v", recovered))
		}
	}()
	value, err := runCallableValue(job.ctx, job.callable, job.args, job.task.cancelCh)
	job.task.complete(value, err)
}
...
func (vm *VM) readLocalTagged(slot int) TaggedValue {
	value := vm.stack[slot]
	if value.typ == TypeOther {
		if c, ok := value.other.(*cell); ok {
			return FromAny(c.value)
		}
	}
	return value
}
...
func (vm *VM) writeLocalTagged(slot int, value TaggedValue) {
	current := vm.stack[slot]
	if current.typ == TypeOther {
		if c, ok := current.other.(*cell); ok {
			c.value = value.ToAny()
			return
		}
	}
...
func (vm *VM) pushTagged(v TaggedValue) error {
	sp := vm.sp
	if sp >= stackLimit {
		return vm.runtimeError("stack overflow")
	}
...
func (vm *VM) popTagged() (TaggedValue, error) {
	sp := vm.sp
	if sp == 0 {
		return TaggedValue{}, vm.runtimeError("stack underflow")
	}
	sp--
	tv := vm.stack[sp]
	vm.stack[sp] = TaggedValue{}
...
var standardModules = map[string]nativeModuleValue{
	...
	"tuxiler": {
		name: "tuxiler",
		members: map[string]builtinValue{
			"build": namedBuiltin("tuxiler.build", func(args []Value) (Value, error) {
				if len(args) < 1 || len(args) > 2 {
					return nil, fmt.Errorf("tuxiler.build expects 1 or 2 arguments")
				}
				targetInput, ok := args[0].(string)
				if !ok {
					return nil, fmt.Errorf("tuxiler.build expects string target path")
				}
				sourcePath, err := resolveTuxSourcePath(targetInput)
				if err != nil {
					return nil, err
				}
				...
				outPath := strings.TrimSuffix(sourcePath, filepath.Ext(sourcePath))
				if len(args) == 2 {
					explicitOut, ok := args[1].(string)
					if !ok {
						return nil, fmt.Errorf("tuxiler.build expects string output path as second argument")
					}
					outPath = explicitOut
				}
				// Native build target is always the currently running runtime OS.
				if runtime.GOOS == "windows" && strings.ToLower(filepath.Ext(outPath)) != ".exe" {
					outPath += ".exe"
				}
				...
				if _, err := writeEmbeddedProgramBinary(string(sourceBytes), absOut); err != nil {
					return nil, err
				}
				return absOut, nil
			}),
			"self_build": namedBuiltin("tuxiler.self_build", func(args []Value) (Value, error) {
				return tuxilerSelfBuild(args)
			}),
		},
	},
	"gui": {
...
var tuxilerMarker = []byte("TUXLANG_EMBEDDED_V1")

func resolveTuxSourcePath(entry string) (string, error) {
	if strings.TrimSpace(entry) == "" {
		return "", fmt.Errorf("path is empty")
	}
	cleaned := filepath.Clean(entry)
	if filepath.IsAbs(cleaned) {
		if pathIsFile(cleaned) {
			return cleaned, nil
		}
		return "", fmt.Errorf("file %q not found", cleaned)
	}
	if pathIsFile(cleaned) {
	...
	nameOnly := filepath.Base(cleaned)
	cwd, err := os.Getwd()
	if err != nil {
		return "", err
	}
	...
	if found == "" {
		return "", fmt.Errorf("could not find %q from %q", entry, cwd)
	}
...
func writeEmbeddedProgramBinary(source string, outputPath string) (string, error) {
	...
	if strings.EqualFold(exeAbs, outAbs) {
		return "", fmt.Errorf("refusing to overwrite currently running executable; choose a different output path")
	}
	runtimeBytes, err := os.ReadFile(exePath)
	if err != nil {
		return "", err
	}
	sourceBytes := []byte(source)
	payload := make([]byte, 0, len(runtimeBytes)+len(sourceBytes)+len(tuxilerMarker)+8)
	payload = append(payload, runtimeBytes...)
	payload = append(payload, sourceBytes...)
	payload = append(payload, tuxilerMarker...)
	lenFooter := make([]byte, 8)
	binary.LittleEndian.PutUint64(lenFooter, uint64(len(sourceBytes)))
	payload = append(payload, lenFooter...)
	if runtime.GOOS == "windows" {
		opts := parseBuildOptionsFromSource(source)
		payload = patchWindowsSubsystem(payload, opts.showConsole)
	}

	if err := os.WriteFile(outAbs, payload, 0755); err != nil {
		return "", err
	}
	return outAbs, nil
}

func parseBuildOptionsFromSource(source string) scriptBuildOptions {
	opts := scriptBuildOptions{showConsole: true}
	...
	for _, rawLine := range lines {
		...
		opts.showConsole = value
	}
	return opts
}

func patchWindowsSubsystem(exe []byte, showConsole bool) []byte {
	if len(exe) < 0x40 {
		return exe
	}
	peOff := int(binary.LittleEndian.Uint32(exe[0x3c:0x40]))
	if peOff < 0 || peOff+0x1e >= len(exe) {
		return exe
	}
	if peOff+4 >= len(exe) || string(exe[peOff:peOff+4]) != "PE\x00\x00" {
		return exe
	}
	optOff := peOff + 4 + 20
	if optOff+0x46 > len(exe) {
		return exe
	}
	var subsystem uint16 = 3
	if !showConsole {
		subsystem = 2
	}
	binary.LittleEndian.PutUint16(exe[optOff+0x44:optOff+0x46], subsystem)
	return exe
}

func tuxilerSelfBuild(args []Value) (Value, error) {
	if len(args) > 1 {
		return nil, fmt.Errorf("tuxiler.self_build expects 0 or 1 argument")
	}
	if len(os.Args) < 2 {
		if strings.TrimSpace(currentProgramSource) == "" {
			return nil, fmt.Errorf("tuxiler.self_build requires a file-based run: tuxlang <file.tux>")
		}
		outPath := "self_build"
		if strings.TrimSpace(currentProgramPath) != "" && currentProgramPath != "__embedded__" {
			outPath = strings.TrimSuffix(currentProgramPath, filepath.Ext(currentProgramPath)) + "_self_build"
		}
		if len(args) == 1 {
			customOut, ok := args[0].(string)
			if !ok {
				return nil, fmt.Errorf("tuxiler.self_build expects string output path")
			}
			outPath = customOut
		}
		if runtime.GOOS == "windows" && strings.ToLower(filepath.Ext(outPath)) != ".exe" {
		...
		if _, err := writeEmbeddedProgramBinary(currentProgramSource, absOut); err != nil {
			return nil, err
		}
		return absOut, nil
	}
	sourcePath, err := resolveTuxSourcePath(os.Args[1])
	if err != nil {
		return nil, err
	}
	sourceBytes, err := os.ReadFile(sourcePath)
	...
	outPath := strings.TrimSuffix(sourcePath, filepath.Ext(sourcePath)) + "_self_build"
	if len(args) == 1 {
		customOut, ok := args[0].(string)
		if !ok {
			return nil, fmt.Errorf("tuxiler.self_build expects string output path")

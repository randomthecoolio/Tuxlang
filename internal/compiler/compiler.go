...
func (c *Compiler) Compile(program *ast.Program) (*bytecode.Function, error) {
	...
	return NewBytecodeOptimizer().Optimize(fn), nil
}
...
// BytecodeOptimizer performs AOT (Ahead-Of-Time) optimizations on bytecode.
...
func NewBytecodeOptimizer() *BytecodeOptimizer {
...
func (o *BytecodeOptimizer) Optimize(fn *bytecode.Function) *bytecode.Function {
...
func (o *BytecodeOptimizer) peepholeOptimize(code []byte) []byte {
	...
	for i := 0; i < len(code); i++ {
		...
		if (op == bytecode.OpTrue || op == bytecode.OpFalse) && i+1 < len(code) && bytecode.Opcode(code[i+1]) == op {
...
func NewStringIntern(maxSize int) *StringIntern {
	return &StringIntern{intern: make(map[string]string), maxSize: maxSize}
}
...
func NewValueCache() *ValueCache { return &ValueCache{cache: make(map[string]interface{})} }
...
func NewInstructionStats() *InstructionStats {
	return &InstructionStats{stats: make(map[byte]int64)}
}
...
func NewCompilationCache(maxSize int) *CompilationCache {
	return &CompilationCache{cache: make(map[string][]byte), maxSize: maxSize}
}
